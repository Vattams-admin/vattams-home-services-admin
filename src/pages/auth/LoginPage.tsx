...
export default function LoginPage() {
  ...
  const [role, setRole] = useState<Role>(() => {
    ...
    return params.get('role') === 'technician' ? 'technician' : 'customer'
  })
        ...
        <Card className="shadow-xl">
          ...
          <div className="flex flex-col items-center gap-2 px-6 pb-6">
            <p className="text-sm text-slate-600">
              ...
              <Link to="/register" className="font-semibold text-orange-600 hover:text-orange-700">Sign up</Link>
            </p>
            <p className="text-xs text-slate-400">Need help? Call us at <span className="font-medium text-slate-600">8189800757</span></p>
          </div>
        </Card>

        <p className="mt-6 text-center text-xs text-slate-400">&copy; {new Date().getFullYear()} VATTAMS Home Services. All rights reserved.</p>
      </div>
    </div>
  )
}
