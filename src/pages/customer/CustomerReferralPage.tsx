...
export function CustomerReferralPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!code) {
        const newCode = `VATTAM${profile.id.slice(0, 6).toUpperCase()}`
  ...
  const shareLink = `${window.location.origin}/register/customer?ref=${referralCode?.code || ''}`
      ...
      <Card className="mb-6 bg-gradient-to-br from-blue-50 to-purple-50">
        <CardContent className="p-6 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            <a href={whatsappSupportLink(`Use my referral code ${referralCode?.code} to get a discount on VATTAMS Home Services! ${shareLink}`)} target="_blank" rel="noopener noreferrer">
