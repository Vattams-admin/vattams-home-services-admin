...
export default function TechnicianReferralPage() {
  ...
  const handleShare = async () => {
    ...
    const shareText = `Join VATTAMS Home Services as a technician! Use my referral code ${referralCode.code} to get started. ${shareUrl}`
    if (navigator.share) {
      try {
  ...
  const handleGenerateCode = async () => {
    ...
    setGenerating(true)
    try {
      const code = `VATTAM${profile?.name?.slice(0, 3).toUpperCase() || 'TEC'}${Math.random().toString(36).slice(2, 6).toUpperCase()}`
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Invite friends to join VATTAMS as technicians and earn {formatCurrency(REFERRAL_REWARD)} for each
          successful referral.
        </p>
      </div>
