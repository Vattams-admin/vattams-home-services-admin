...
export type Invoice = { id: string; invoice_number: string; booking_id: string; customer_id: string; technician_id: string | null; service_name: string; amount: number; status: string; payment_met[...]
...
export type ServiceArea = { id: string; city: string; city_ta: string | null; district: string; pincode: string | null; is_active: boolean; state: string | null; taluk: string | null; area: string[...]
...
export type TechnicianWallet = { id: string; technician_id: string; balance: number; verification_fee_paid: boolean; verification_fee_amount: number; verification_fee_paid_at: string | null; refun[...]
...
export type VerificationPayment = { id: string; technician_id: string; amount: number; payment_method: string; payment_status: string; transaction_id: string | null; payment_date: string | null; c[...]
export type Refund = { id: string; technician_id: string; amount: number; refund_type: string; refund_method: string | null; status: 'pending' | 'approved' | 'rejected' | 'processed' | 'completed'[...]
...
export type CustomerFollowup = { id: string; customer_id: string; admin_id: string | null; scheduled_date: string; reason: string; status: string; notes: string | null; completed_at: string | null[...]
export type CustomerComplaint = { id: string; customer_id: string; booking_id: string | null; complaint_type: string; description: string; status: string; resolution: string | null; created_at: st[...]
...
export type Referral = { id: string; referrer_id: string; referred_email: string | null; referred_name: string | null; referral_code: string; referral_type: string; status: string; reward_amount: [...]
...
export type PopupAnnouncement = { id: string; title: string; content: string; type: string; is_active: boolean; start_date: string | null; end_date: string | null; dismissible: boolean; created_at[...]
export type MarketingCampaign = { id: string; name: string; type: string; channel: string; target_audience: string | null; status: string; start_date: string | null; end_date: string | null; metri[...]
...
export type GoogleBusinessProfile = { id: string; business_name: string; business_description: string | null; address: string | null; service_areas: string | null; working_hours: string | null; co[...]
export type AnalyticsEvent = { id: string; event_name: string; event_category: string; page_url: string | null; user_id: string | null; session_id: string | null; metadata: Record<string, unknown>[...]
export type AnalyticsSettings = { id: string; ga4_measurement_id: string | null; ga4_api_secret: string | null; meta_pixel_id: string | null; meta_access_token: string | null; gsc_verification_tok[...]
export type EmailCampaign = { id: string; name: string; subject: string; template: string; recipient_type: string; status: string; sent_count: number; open_count: number; click_count: number; crea[...]
...
export type CustomerReview = { id: string; customer_name: string; rating: number; review_text: string; service_name: string | null; is_featured: boolean; is_approved: boolean; source: string; crea[...]
